document.addEventListener("DOMContentLoaded", function () {
    ...
    function saveState() {
        ...
        localStorage.setItem("tdsHeaders", JSON.stringify(state.headers));
    }
    ...
    function initializeSpreadsheet() {
        const savedData = JSON.parse(localStorage.getItem("tds")) || [[]];
        const cellLockStates = JSON.parse(localStorage.getItem("cellLockStates")) || {};
        const savedHeaders = JSON.parse(localStorage.getItem("tdsHeaders")) || []; // Updated to retrieve headers from the new key
    ...
    function undo() {
        if (undoStack.length > 1) {
            ...
            localStorage.setItem("ZP", JSON.stringify(previousState.data));
            localStorage.setItem("tdsHeaders", JSON.stringify(previousState.headers)); // Updated to save headers
    ...
    function redo() {
        if (redoStack.length > 0) {
            ...
            localStorage.setItem("tds", JSON.stringify(nextState.data));
            localStorage.setItem("tdsHeaders", JSON.stringify(nextState.headers)); // Updated to save headers
        ...
        for (let i = 0; i < colCount; i++) {
            ...
            th.addEventListener('input', () => {
                ...
                const headers = Array.from(headerRow2.children)
                    .slice(1)
                    .map(header => header.textContent);
                localStorage.setItem("tdsHeaders", JSON.stringify(headers)); // Updated to save headers
            });
    ...
    function saveData() {
        ...
        // Save data to localStorage with specific keys
        localStorage.setItem("tds", JSON.stringify(data));
