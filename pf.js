document.addEventListener("DOMContentLoaded", function () {
    ...
    function initializeSpreadsheet() {
        const savedData = JSON.parse(localStorage.getItem("pf")) || [[]];
        const cellLockStates = JSON.parse(localStorage.getItem("cellLockStates")) || {};
    ...
    function undo() {
        if (undoStack.length > 1) {
            ...
            localStorage.setItem("pf", JSON.stringify(previousState.data));
    ...
    function redo() {
        if (redoStack.length > 0) {
            ...
            localStorage.setItem("pf", JSON.stringify(nextState.data));
